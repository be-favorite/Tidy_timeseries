...
## **1 Preparing**
***
tidyverts ecosystem을 이루는 대부분의 패키지들은 [Introduction](https://github.com/be-favorite/tidyverts)에서 말했듯이, {fpp3}으로 불러올 수 있습니다. {tsibbletalk}은 {shiny}와 함께 동작하는 반응형 그래픽을 제공하는 패키지로 본 튜토리얼에서는 생략하겠습니다:
```{r}
library(fpp3)
```
...
loaded_package <- c("fable.prophet", "nycflights13", "purrr", "shiny")
...
### 2.3 Aggregate over calendar periods{#anchor2}
...
```{r forecast, warning = FALSE, out.width = "80%", fig.align = "center"}
fc %>% 
...
## **6 fable.prophet **
***
{fable.prophet}은 facebook에서 제안한 단일 시계열 예측모형에 대한 적합 또한 tidy한 인터페이스로 제공해줍니다. prophet 모형은 시계열의 시간 종속적인 특성을 고려하는 기존의 시계열 모형(e.g. 지수평활법, ARIMA 모형)과 달리 curve-fitting(e.g. [splines](https://be-favorite.tistory.com/56?category=923110))으로 모형을 적합하며, 시계열을 다음과 같이 세 가지 요소로 분해하고 각 요소를 시간의 함수로 가법적으로 모형화합니다.
...
각 계열에 대해 prophet 모형이 잘 적합된 것을 확인할 수 있습니다. 적합된 모형의 각 요소들은 `components()`로 추출할수 있습니다:
```{r}
components(fit)
```
...
연별 계절패턴의 경우 주별로 대개 비슷하나, 북방 지역(the Northern Territory)의 경우 다른 주들과는 크게 다른 계쩔 패턴을 보여주고 있습니다. 마지막으로, prophet 모형의 예측도 `forecast()`를 이용해 쉽게 수행할 수 있습니다. 향후 2년에 대해 예측해보았습니다:
```{r forecast_prophet, warning = FALSE, message = FALSE, out.width = "80%", fig.align = "center"}
fc <- fit %>% 
  forecast(h = 24)
cafe %>% 
...
[Forecasting: Principles and Practices (3rd Ed.)](https://otexts.com/fpp3/)에서는 prophet 모형외에도, 벡터 자기회귀모형, 인공신경망 기반의 시계열 예측모형, 붓스트랩 및 배깅 기법을 활용한 시계열 예측 모형 등의 고급 시계열 예측 모형도 제공해줍니다. fpp3을 참고해보시기 바랍니다.

## **7 Summary**
***
tidyverts ecosystem이 전반적으로 작동하는 과정을 소개해 보았습니다. 그러나, 시계열 자료의 예측 모델링 대한 깊은 내용과 더불어 tidyverts를 좀 더 디테일하게 활용하기 위해서는 [Forecasting: Principles and Practices (3rd Ed.)](https://otexts.com/fpp3/)을 천천히 읽어볼 필요가 있습니다. 저도 시간이 날 때마다 틈틈히 읽고 [깃허브](https://github.com/be-favorite)에 정리해보려고 합니다. tidyverse와 [tidymodels](https://github.com/be-favorite/Tutorial_tidymodels)를 통해 자료의 전처리 시간, 예측 모형의 개발 시간을 크게 단축시켰듯이, fpp3을 통해 tidyverts ecosystem을 어느정도 마스터하면 시계열 예측 모델링에 대한 시간을 상당히 단축시킬 수 있을 것 같네요. 빠르고 정확하면서 디테일한 자료 분석을 위해서는 끊임없는 R 역량 개발 및 공부가 답이라고 생각합니다.`r emo::ji("blush")`
